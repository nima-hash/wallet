...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            switch ($action) {
                case 'updateExpense':
                    ...
                    if ($expense->update($setValues, $condition, $params)) {
                        ...
                        sendOutput(json_encode(['success' => true, 'message' => 'the Record was successfully updated.']),  ['Content-Type: application/json', 200]);
        ...
        switch ($action) {
            case 'getCategories':
                $expenseList = $expense->getCategories();
                break;
            case 'getCategory':
                $expenseList = $expense->getCategory($params);
                break;
            default:
        ...
        if (count($expenseList)>=0) {
            sendOutput(json_encode(['success' => true, 'data' => $expenseList]),  ['Content-Type: application/json', 200]);
            // sendOutput($expenseList, []);
        }
    ...
    // }
    $expense->get('db')->sendOutput(json_encode(array('success' => false, 'error' => $e->getData(), 'code' => (int)$e->getCode())),
        array('Content-Type: application/json',(int)$e->getCode() ?: 500)
    );
} catch (Exception $e) {
    $expense->get('db')->sendOutput(json_encode(array('success' => false, 'error' => $e->getMessage(), 'code' => (int)$e->getCode())),
        array('Content-Type: application/json',(int)$e->getCode() ?: 500));
}
